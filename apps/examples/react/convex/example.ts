...
import { exposeApi } from "@orbit-kit/auth-sdk";
...
export const addComment = mutation({
  ...
  handler: async (ctx, args) => {
    return await ctx.runMutation(components.authSdk.lib.add, {
      text: args.text,
...
export const listComments = query({
  ...
  handler: async (ctx, args) => {
    return await ctx.runQuery(components.authSdk.lib.list, {
      targetId: args.targetId,
    });
  },
});
...
export const translateComment = action({
  ...
  handler: async (ctx, args) => {
    return await ctx.runAction(components.authSdk.lib.translate, {
      baseUrl: BASE_URL,
...
// the component's API:
export const { list, add, translate } = exposeApi(components.authSdk, {
